...
mrb_value hiro_scene_mrb_update(mrb_state* mrb, mrb_value self) {
  hiro_scene_each_child_call(mrb, self, "update", 0, NULL);
  return self;
}

mrb_value hiro_scene_mrb_draw(mrb_state* mrb, mrb_value self) {
  hiro_scene_each_child_call(mrb, self, "draw", 0, NULL);
  return self;
}

void hiro_define_scene(mrb_state* mrb) {
  struct RClass* klass = mrb_define_class(mrb, "Scene", mrb->object_class);

  mrb_define_method(mrb, klass, "add", hiro_scene_mrb_add, MRB_ARGS_REQ(1));
  mrb_define_method(mrb, klass, "update", hiro_scene_mrb_update, MRB_ARGS_NONE());
  mrb_define_method(mrb, klass, "draw", hiro_scene_mrb_draw, MRB_ARGS_NONE());
}
